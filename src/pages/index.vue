...
export default class ProductPage extends Vue {
  ...
  get watchProperties() {
    if (!this.page) return null;
      ...
      return this.page.currentPage.toString() + this.page.sortField + this.page.sortDirection;
    }
  }
  ...
  async onWatchPropertiesChanged(val: string, oldVal: string) {
    ...
    await this.productModule.findAllPaged({
      page: page,
      size: size,
      sortField: sortField,
    ...
    if (sortField && sortDirection)
      this.$router.push({
        path: '/',
        query: { page: page.toString(), size: size.toString(), sortField: sortField, sortDirection: sortDirection }
      });
    else this.$router.push({ path: '/', query: { page: page.toString(), size: size.toString() } });
  }
}
</script>
